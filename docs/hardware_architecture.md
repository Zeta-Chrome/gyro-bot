...
flowchart TD
    ...
    %% MAIN POWER SOURCE
    Battery["LiPo 3S Battery\n11.6V 2200mAh"]

    %% BUCK CONVERTERS
    Buck5V["Buck Converter #1\n5V @ 6A"]
    Buck10V["Buck Converter #2\n10.5V Motor Rail"]

    %% MAIN PROCESSING UNITS
    ESP32["ESP32 (Main MCU)\n- PID Balance\n- Motor Ctrl\n- Sensor Fusion\n- Networking"]
    ESP32CAM["ESP32-CAM\nOV7670 Camera\n- Frame Streaming\n- Visual Odometry"]
    FPGA["FPGA Co-Processor\n- Object Detection\n- SLAM\n- Edge Compute"]
    ...
    %% ACTUATORS
    Servo["MG90 Servo\n(Camera Pitch)"]
    MotorDriver["DRV8833 Motor Driver\nDual H-Bridge"]
    MotorL["Left Motor\n10.5V"]
    MotorR["Right Motor\n10.5V"]

    %% POWER REGULATION INSIDE ESP32
    Reg3V3["ESP32 Internal 3.3V Regulator"]

    %% CAMERA GIMBAL
    Gimbal["Camera Gimbal Mount"]
    ...
    Buck10V --> MotorDriver --> MotorL
    MotorDriver --> MotorR

    %% SIGNAL CONNECTIONS
    ESP32 -->|I2C| IMU
    ESP32 -->|GPIO Trigger/Echo| Ultrasonic
    ESP32 -->|PWM| MotorDriver
    ESP32 -->|PWM| Servo
    ESP32 -->|High-Speed Link| FPGA
    ESP32CAM -->|Video Stream| ESP32
    Servo --> Gimbal
